    ...
    for (auto title : m_map.keys()) {
        ...
        for (auto purchase : purchasesList) {
            ...
            for (auto purchase : purchasesList) {
                ...
                currency = purchase.propertyByName(Property::CURRENCY_OF_PROCEEDS).stringValue();
                numberOfItems += units;
            ...
            appendLineToModel(QString::number(counter), title, numberOfItems, value, currency);
            counter++;
        }
...
void BalanceSheet::appendLineToModel(const QString &vHeader, const QString &title, const QString &country,
